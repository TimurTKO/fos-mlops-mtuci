# Дисциплина «MLOps и инженерия жизненного цикла ML-систем» — студенческий комплект материалов

> **Это демонстрационный студенческий комплект материалов.** Ветка показывает рекомендуемый состав материалов, которые преподаватель может выдавать обучающимся. Ключи ответов, эталонные решения и закрытые преподавательские материалы в неё не включены.

## О назначении этой ветки

- **Это демонстрационный студенческий комплект материалов.** Он показывает рекомендуемый состав выдачи, а не является готовой рабочей средой конкретного курса.
- **Преподаватель должен адаптировать её под свою инфраструктуру:** доступные вычислительные ресурсы, порты, наличие Docker, используемую LMS, систему контроля версий и правила образовательной организации.
- **Преподаватель формирует собственную копию или закрытую выдачу.** Для фактического проведения дисциплины создаётся отдельный репозиторий или закрытая выдача в инфраструктуре образовательной организации.
- **Критерии оценивания открыты.** Рубрики модулей, шкала зачёта и обязательные условия доступны обучающемуся с самого начала.
- **Ключи и эталонные решения исключены.** Состав исключений зафиксирован в манифесте [`.student-exclude`](.student-exclude) и проверяется скриптом [`tools/check_student_delivery.py`](tools/check_student_delivery.py).

Репозиторий является публичным. Ветка `student` не ограничивает технически доступ к полной экспертно-преподавательской версии в ветке [`main`](https://github.com/TimurTKO/fos-mlops-mtuci/tree/main): это демонстрация рекомендуемого состава выдачи, а не механизм разграничения доступа. Разграничение обеспечивается только собственной копией репозитория или закрытой выдачей на стороне преподавателя.

## Что исключено из комплекта
...
- готовые решения лабораторных работ в составе учебного проекта;
- эталонный инструмент технической проверки полной версии.

## Что входит в комплект

- рабочая программа дисциплины, ФОС и модель измерения;
- задания девяти лабораторных работ и рубежных защит;
- критерии оценивания и открытые рубрики;
- пустые шаблоны отчётов, прослеживаемости, постмортема и политики выпуска;
- методические указания для обучающихся;
- стартовые материалы: исходный ноутбук, генератор синтетических данных, версии данных и сценарии инцидентов;
- контракт данных и конфигурационные шаблоны;
- короткие тесты №1–9 без ключа;
- каркас учебного проекта с сохранёнными структурой и интерфейсами.
...
- [Каркас учебного проекта](resources/starter-project/project-baseline/README.md)
...
[`resources/starter-project/project-baseline`](resources/starter-project/project-baseline/README.md) содержит каркас учебного проекта: исходный ноутбук, генератор данных, контракт данных, конфигурационные шаблоны и модули с сохранёнными интерфейсами и комментариями `TODO`. Реализация выстраивается последовательно в ходе лабораторных работ.
...
### 4.2. Что вы получаете на старте
...
Итоговый проект проверяет полный жизненный цикл ML-системы. Перед защитой фиксируется Git SHA/тег, заполняются чек-лист сдачи и декларация использования ИИ. Индивидуальная защита включает воспроизводимую демонстрацию, вопросы по основной и смежной зонам и небольшое живое изменение. Конкретный вопрос и вариант живого изменения выбирает преподаватель; заранее они не сообщаются.
...
## 7. Проверка состава комплекта

```bash
python tools/check_student_delivery.py
```

Скрипт подтверждает, что комплект соответствует манифесту [`.student-exclude`](.student-exclude): ключи ответов и эталонные решения отсутствуют, обязательные учебные материалы на месте.
